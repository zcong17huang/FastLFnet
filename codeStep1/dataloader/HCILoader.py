...
def load_LFdata(LF_dir, loader, dploader):
    ...
    return img_0d, img_90d, data_disp
...
class myImageFloder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_0d, img_90d, disp = load_LFdata(img_dir, loader=self.loader, dploader=self.dploader)

        disp = np.ascontiguousarray(disp, dtype=np.float32)

        if self.iftrain == True:
            input_0d, input_90d, disp_batch_label = preprocess.data_crop(img_0d, img_90d, disp, self.cropsize, class_name,
                                                      self.boolmask_img4, self.boolmask_img6, self.boolmask_img15)
            input_0d, input_90d, disp_batch_label = preprocess.data_augment(input_0d, input_90d, disp_batch_label)

        else:
            input_0d, input_90d, disp_batch_label = preprocess.test_dataprocess(img_0d, img_90d, disp, self.cropsize, self.id_start)
        ...
        return center_input, view1_input, view2_input, view3_input, view4_input, disp_batch_label
