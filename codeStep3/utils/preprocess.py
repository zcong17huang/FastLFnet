...
def data_crop(img_0d, img_90d, disp, edge, input_size, class_name, boolmask_img4, boolmask_img6, boolmask_img15):
    sum_diff = 0
    edge_diff = 0
    valid = 0

    # when it is an untextured or reflective region：
    while (sum_diff < 0.01 * input_size * input_size or edge_diff < 0.008 * input_size * input_size or valid < 1):
        ...
        if valid > 0:
            edge_center = np.squeeze(edge[idx_start: idx_start + scale * input_size:scale,
                                          idy_start: idy_start + scale * input_size:scale].astype('float32'))
            edge_diff = np.sum(edge_center)
            if edge_diff >= 0.008 * input_size * input_size:
                ...
                if sum_diff >= 0.01 * input_size * input_size:
                    ...
                    edge_batch_label = (1.0 / scale) * edge[idx_start: idx_start + scale * input_size:scale,
                                                            idy_start: idy_start + scale * input_size:scale]

    input_0d = np.float32((1 / 255.) * input_0d)
    input_90d = np.float32((1 / 255.) * input_90d)

    return input_0d, input_90d, disp_batch_label, edge_batch_label  # shape = [input_size, input_size, 9]


def data_augment(input_0d, input_90d, disp_batch_label, edge_batch_label):
    """
    ...
    if transp_rand == 1:
        ...
        edge_batch_label = np.copy(np.transpose(edge_batch_label, (1, 0)))
    ...
    if rotation_rand == 1:
        ...
        edge_batch_label = np.copy(np.rot90(edge_batch_label, 1))
    """ 180 """
    if rotation_rand == 2:
        ...
        edge_batch_label = np.copy(np.rot90(edge_batch_label, 2))
    """ 270 """
    if rotation_rand == 3:
        ...
        edge_batch_label = np.copy(np.rot90(edge_batch_label, 3))
    ...
    return input_0d, input_90d, disp_batch_label, edge_batch_label


def test_dataprocess(img_0d, img_90d, disp, edge_binary, input_size, id_start):
    ...
    edge_binary_label = edge_binary[id_start: id_start + input_size, id_start: id_start + input_size]
    ...
    return input_batch_0d, input_batch_90d, disp_batch_label, edge_binary_label
