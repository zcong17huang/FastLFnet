...
parser.add_argument('--loadmodel_1', default='./checkpoints/FastLFnet_model.tar', help='load model')
parser.add_argument('--loadmodel_2', default='./checkpoints/FastLFnet_model_edge.tar', help='load model')
parser.add_argument('--savemodel', default='./checkpoints/FastLFnet_model_final.tar', help='save model')
...
def main():
    ...
    for key, v in model.named_parameters():
        if re.match(r'module.feature_extraction', key):
            params += [{'params': v, 'lr': base_lr * 0.04, 'name': key}]
        else:
    ...
    state_dict_1 = torch.load(args.loadmodel_1)
    pretrained_dict_1 = state_dict_1['state_dict']
    pretrained_dict_1 = {k: v for k, v in pretrained_dict_1.items() if k in model_dict}
    model_dict.update(pretrained_dict_1)

    state_dict_2 = torch.load(args.loadmodel_2)
    pretrained_dict_2 = state_dict_2['state_dict']
    ...
    log_train.info("=> loaded checkpoint '{}, {}' (epoch {})".format(args.loadmodel_1, args.loadmodel_2, epoch_start))
...
def train_model(device, model, scheduler, TrainImgLoader, optimizer, length_train):
    ...
    for batch_idx, (center_input,
                    ...
                    disp_batch_label, edge_binary_label) in enumerate(TrainImgLoader):
        # --------------------------------------------------
        optimizer.zero_grad()
        ...
        output, edge_out = model(center_input, view1_input, view2_input, view3_input, view4_input)

        output = 25 * output + 100
        disp_batch_label = 25 * disp_batch_label + 100                # Scale the disparity range to [0,200]

        loss1 = nn.SmoothL1Loss()
        loss2 = losses.GradientLoss(device)
        # loss2 = losses.DispEdgeLoss(device)
        ...
        loss = loss1(disp_batch_label, output) + \
               2.2 *loss2(disp_batch_label, output) +\
               100.0 * cross_entropy_loss2d(edge_out, edge_binary_label, device)
